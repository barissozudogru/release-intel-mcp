...
### Fixed
- Both tools categorize a pull request by its own title, so they no longer disagree about the same PR.

### Added
- `release` category for release PRs, with a bucket in both tools.
...
- `get_release_summary` tool: generate a structured release context object combining commits, PRs, contributors, and aggregate statistics for AI synthesis
